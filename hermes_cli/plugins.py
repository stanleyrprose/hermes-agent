...
class PluginManifest:
    ...
    provides_skills: List[str] = field(default_factory=list)  # NEW: skill names bundled with this plugin
...
class LoadedPlugin:
    ...
    skills_registered: List[str] = field(default_factory=list)  # NEW: skills bundled with this plugin
    enabled: bool = False
...
class PluginContext:
    ...
    def register_skill(
        ...
        """Register a skill in the SkillRegistry **and** track it as plugin-provided.

        The skill file should be located relative to the plugin directory.
        """
        from agent.skill_registry import SkillRegistry, SkillEntry, SkillReadinessStatus
        from pathlib import Path

        registry = SkillRegistry()
        skill_path = Path(self.manifest.path).parent / path if self.manifest.path else Path(path)
        ...
        entry = SkillEntry(
            ...
            metadata=metadata or {"bundled_by_plugin": self.manifest.name},
        )
        registry.register(entry)
        self._manager._plugin_skill_names.add(name)
        logger.debug("Plugin %s registered skill: %s", self.manifest.name, name)
...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        self._plugin_skill_names: Set[str] = set()  # NEW: skill names registered by plugins
        ...
        for child in sorted(path.iterdir()):
            ...
            try:
                ...
                manifest = PluginManifest(
                    ...
                    provides_skills=data.get("provides_skills", []),
                    source=source,
                    path=str(child),
                )
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # Compute skills newly registered by this plugin (not by previous plugins)
                loaded.skills_registered = [
                    s for s in self._plugin_skill_names
                    if s not in {
                        n
                        for name, p in self._plugins.items()
                        for n in p.skills_registered
                    }
                ]
...
def get_plugin_skill_names() -> Set[str]:
    """Return the set of skill names registered by plugins."""
    return get_plugin_manager()._plugin_skill_names
